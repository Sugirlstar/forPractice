...
#设置图片存储位置及名称
setwd("C:/Users/Yaoying/Desktop/2108论文收尾/211001定稿/drought_draft")
ctype = "drought-spi90--1-10days-0-5days"
#加载必要包
library(tcltk)
library(raster)
library(sp)
...
#####
#01识别连续da天以上1
#查找连续的10个1，满足这个条件的1保存，不满足的全部改为0
...
for(x in 1:xx)
{
  for(y in 1:yy)
    ...
    {
      Flag[ x,y, st-1 ]=0 #第89天置零，从第89天起识别0/1组合
      for( i in st:days )
        if(Flag[x,y,i] == 1 & Flag[x,y,i-1] != 1)
        {
          duration=sumfun(Flag[x,y,],i)
          if(duration < da) #低于da天的都剔除
...
#03设定Flag3为次数，即仅保留每次干旱事件第一天为1，其他全部置为0
...
save.image("C:/Users/Yaoying/Desktop/2108论文收尾/211001定稿/drought_draft/干旱计算全流程.RData")
