...
#设置图片存储位置及名称
setwd("C:/Users/Yaoying/Desktop/2108论文收尾/211001定稿/heatwave_draft")
...
dates=substring(dimnames(TX)[[3]],5,8) #获取月日字符串
fdates=levels(factor(dates)) #获取字符串因子（366天）
nday = 7 #共15日滑动窗口
...
for(x in 1:xx)
  for(y in 1:yy)
    if (TRUE %in% (TX[x, y, ] >= -999))
      for(i in 1:366)
      {
        today = fdates[i] #第i天的月日字符串
        f <- which(dates == today) #f:每年的第i天在时间序列上的位置
        ys=TX[x,y,f]
        tdays = f + rep(-nday:nday,each=LL)
        ff=subset(tdays,tdays<=days & tdays>0) #剔除超出长度范围的日数
        ymean=mean(TX[x,y,ff],na.rm=TRUE)
        ysd=sd(TX[x,y,ff],na.rm=TRUE)
        # 不用前后15日值纳入计算
        # ymean=mean(TX[x,y,f],na.rm=TRUE)
        # ysd=sd(TX[x,y,f],na.rm=TRUE)
...
#save.image("C:/Users/Yaoying/Desktop/2108论文收尾/211001定稿/heatwave_draft/热浪_STI结果.RData")
#setwd("C:/Users/Yaoying/Desktop/2108论文收尾/211001定稿")
#save(TSI, file = "STI.RData")

#03-01 Defining Flags 
#####
#设置旗子
Flag <- array(NA,dim = c(xx,yy,days)) #第一阈值
for(i in st:days)
    Flag[,,i] = ( -TSI[,,i] <= thr1 )  #是-STI<= -1 即STI>= 1
#####
...
for(x in 1:xx)
{
  for(y in 1:yy)
    ...
    {
      Flag2[x,y,st-1]=0
      #Flag2[x,y,which(is.na(Flag2[x,y,])==TRUE)]=0   ###热浪有许多空值，需要手动填上
...
#设定Flag3为次数，即仅保留每次干旱事件第一天为1，其他全部置为0
...
#04 Calculating indicators
#####
#00-02计算逐年频次和、年总历时
SPId=TSI*Flag2
SPId[which(SPId==0)]=NA #要把0值置为NA,否则强度值结果不对

Hy = substring(dimnames(TX)[[3]], 1, 4)
fhy = factor(Hy) #转化为年份因子
DRfre = array(dim = c(xx, yy, length(levels(fhy)))) #定义年频次
DRdur = array(dim = c(xx, yy, length(levels(fhy)))) #定义年历时
DRstg = array(dim = c(xx, yy, length(levels(fhy)))) #定义年历时
...
for (x in 1:xx)
  for (y in 1:yy)
    if (TRUE %in% (TX[x, y,] >= -999))
    {
      DRfre[x, y,] = tapply(Flag3[x, y,], fhy, sum, na.rm = TRUE) #计算年频次总和
      DRdur[x, y,] = tapply(Flag2[x, y,], fhy, sum, na.rm = TRUE) #计算年历时总和
      DRstg[x, y,] = tapply(SPId[x, y,], fhy, mean, na.rm = TRUE) #计算年强度均值
    }
#!!!有的年份历时低于10days,是干旱恰好在年初or年尾，被隔开了
#得到DRfre/DRdur——逐格点逐年频次和、总历时和
...
DRFREr = raster(DRFRE) #转为r层
DRDURr = raster(DRDUR)
DRSTGr = raster(DRSTG)
extent(DRFREr) = c(72, 136, 18, 54) #确定范围
extent(DRDURr) = c(72, 136, 18, 54) #确定范围
extent(DRSTGr) = c(72, 136, 18, 54) #确定范围
crs(DRFREr) = crs(r) #确定投影
crs(DRDURr) = crs(r) #确定投影
crs(DRSTGr) = crs(r) #确定投影

#00-05 计算某年多地平均值（按权重求和）
r1 = raster(DRfre[, , 1])   #随便取一天，取其空间范围
...
for (i in 1:LL)
{
  y1 = DRfre[, , i] * w  #权重×数值得新的数值
  DRFREh[i] = sum(y1, na.rm = T)  #得到均值
  y2 = DRdur[,,i] * w
...
pbt.TSTD = function(objx,ck,ctype) #objx=DRfre, ck = DRFREh 
{
  ...
  if(rst[1]!=0)
  {
    ...
    #tend1
    png(
      file = paste("03TSTD_tend1_",rst[1],"_",ctype,"_",vname,",(+)",posp1,"(-)",negp1,"(++)",xposp1,"(--)",xnegp1,".png", sep = ""),
      width = 1800,
      height = 1400,
      res = 72 * 3
    )
    ...
    #tend2
    png(
      file = paste("03TSTD_tend2_",rst[1],"_",ctype,"_", vname,",(+)",posp2,"(-)",negp2,"(++)",xposp2,"(--)",xnegp2, ".png", sep = ""),
      width = 1800,
      height = 1400,
      res = 72 * 3
    )
...
pbt.MKSD = function(objx,ctype) #objx=DRfre
{
  ...
  png(
    file = paste("08MKSD_",ctype, "_",vname,",(+)",posp,"(-)",negp,"(++)",xposp,"(--)",xnegp, ".png", sep = ""),
    width = 1800,
    height = 1400,
    res = 72 * 3
  )
...
#
save.image("C:/Users/Yaoying/Desktop/2108论文收尾/211001定稿/heatwave_draft/热浪计算全流程.RData")
